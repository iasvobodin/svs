...
void main() {
  ...
  float vertexProgress = smoothstep(startAt, 1., p);

  vec3 transformedPos = pos;
  vec2 transformedUV = aTextureCoord;

  mat2 rotation = rotate2d(vertexProgress*(3.14*2.)*0.25);
  ...
  pos = transformedPos;
    ...
    // Scale
    // uScaleToViewSize
  vec2 scale = vec2(1. + (uScaleVector - 1.) * vertexProgress);
  pos.xy *= scale;
  // Move to center
  ...
  // float noiseFreq = 2.5;
  // float noiseAmp = 3.95; 
  // vec3 noisePos = vec3(pos.x * noiseFreq + p, pos.y, pos.z);
  ...
  vTextureCoord = transformedUV;
  vProgress = vertexProgress;
  vVertexPosition = aVertexPosition;
  vActiveTextureCoord = (activeTexMatrix * vec4(transformedUV, 0.0, 1.0)).xy;
  vNextTextureCoord = (nextTexMatrix * vec4(transformedUV, 0.0, 1.0)).xy;
}
